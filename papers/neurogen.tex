...
\midrule
Mean & 0.9002 & 0.8866 & \textbf{0.8870} & \textbf{0.8868} & $+1.51$\% & $\mathbf{+1.46}$\% & $\mathbf{+1.48}$\% \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{What the data do not support.} Whether F is \emph{better} than B is not resolved, and at this measurement precision it is not resolvable. Only $2$ of $5$ paired differences favour F; the permutation test returns $p = 0.625$, the paired $t$-test $p = 0.771$, and $d_z = -0.14$. The mean difference, $-0.05$\%, is an order of magnitude below the residual noise on this comparison ($0.0023$ bpb, \S\ref{sec:limitations}). We therefore report ``removal costs nothing,'' not ``removal helps.'' An earlier draft of this work reported means of $+1.50$\% for F against $+1.33$\% for B, taken over the two seeds that completed in a three-seed run, and read them as evidence that windows eventually become a ceiling. Those two seeds disagreed in sign; the reading did not survive replication, and the five-seed mean is if anything marginally negative.

\paragraph{Statistical power.} The pre-registered criteria were: claim F-vs-A only on $5/5$ positive differences, and claim F-vs-B only on $\geq 4/5$ positive \emph{and} paired $t$-test $p < 0.05$. The first is met; the second fails, as anticipated---given the residual noise of 0.0023 bpb and a true difference no larger than ${\sim}0.0015$, five seeds gave roughly a 22\% chance of meeting it. Resolving F-vs-B needs a lower-variance endpoint measurement, not more seeds; see item~(4) of \S\ref{sec:limitations}.

\paragraph{The switch is a shock, and the divergence is stochastic.} An earlier three-seed run of this experiment lost one arm-F run to NaN after the switch, at seed 256. It would be natural to read that as a property of that initialization. It is not: in the five-seed replication of Table~\ref{tab:curriculum} the \emph{same} seed completed normally at 0.8865, and no seed diverged. Across both runs the observed rate is one divergence in eight completed arm-F runs, attached to no particular seed.
...
\begin{table}[H]
\centering
\caption{Release-point sweep (20k steps, 3.4M, five matched seeds, 500-step ramp at each release point). $R@x$ releases the windows at step $x$. Arm~A never applies them; arm~B never releases them. Criteria were fixed before the runs (\S\ref{sec:limitations}).}
\label{tab:release}
\begin{tabular}{rccccccl}
\toprule
Seed & A: full & $R@2$k & $R@5$k & $R@10$k & $R@15$k & B: quartic & Best \\
\midrule
42 & 0.9041 & 0.8932 & \textbf{0.8924} & 0.8950 & 0.8956 & 0.8927 & $5$k \\
137 & 0.8913 & \textbf{0.8726} & 0.8741 & 0.8743 & 0.8754 & 0.8788 & $2$k \\
256 & 0.8941 & \textbf{0.8833} & 0.8847 & 0.8867 & 0.8875 & 0.8830 & $2$k \\
789 & 0.9098 & \textbf{0.8855} & 0.8875 & 0.8882 & 0.8890 & 0.8896 & $2$k \\
1337 & 0.9016 & \textbf{0.8863} & 0.8892 & 0.8899 & 0.8915 & 0.8889 & $2$k \\
\midrule
Mean & 0.9002 & \textbf{0.8842} & 0.8856 & 0.8868 & 0.8878 & 0.8866 & \\
vs A & --- & $+1.78$\% & $+1.62$\% & $+1.48$\% & $+1.38$\% & $+1.51$\% & \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The constraint works at every release point tested.} All four columns beat their paired baseline on all five seeds, so each reaches the exact permutation floor $p=1/32=0.031$ ($d_z$ between 2.12 and 2.84). Ten percent of training spent windowed is already enough to obtain the full effect. No run diverged.

\paragraph{Earlier release is better, and the curve is not flat.} We pre-registered a null that all release points would fall within the 0.0023 bpb residual of each other, in which case the recipe would need no tuning. That null is falsified: the spread of means is 0.0036 bpb, about a quarter of the baseline-to-quartic effect. The ordering is monotone---earlier release is better---and identical on every seed for the three later points: $R@2$k beats $R@10$k and $R@15$k on 5/5 seeds (paired $t$-test $p=0.0026$ and $p=0.0019$), and $R@10$k beats $R@15$k on 5/5 ($p=0.0049$).

\paragraph{We do not claim an optimum.} Our criterion for naming a best release point required the winner to beat \emph{every} other point on $\geq4/5$ seeds \emph{and} to clear a paired $t$-test at $p<0.05$ against the runner-up. $R@2$k has the best mean, but against $R@5$k it is 4/5 seeds with $p=0.0836$, so the criterion is not met and 2k and 5k are not separated by these data. The honest statement is that release should be \emph{early}---somewhere in the first 10--25\% of training---and that we cannot resolve where in that range.

\paragraph{Releasing still does not beat retaining.} Even the best release point is not distinguishable from keeping the windows throughout: $R@2$k versus arm~B is 3/5 seeds, $p=0.125$, mean $+0.0024$ bpb. This is the same conclusion Table~\ref{tab:curriculum} reached at the halfway point, and it is the reason the recipe is justified by deployment convenience---a standard architecture at inference---rather than by a quality gain.
...
\begin{table}[H]
\centering
\caption{The release curve with a fixed 500-step ramp (20k steps, 3.4M, five matched seeds). $R@x$ releases the windows at step $x$; every point here beats its paired baseline on 5/5 seeds. This sweep found no lower bound; \S\ref{sec:floor} goes below it and does.}
...
\midrule
Mean bpb & \textbf{0.8830} & 0.8833 & 0.8846 & 0.8842 & 0.8856 & 0.8868 & 0.8878 \\
vs baseline & $\mathbf{+1.91}$\% & $+1.87$\% & $+1.73$\% & $+1.78$\% & $+1.62$\% & $+1.48$\% & $+1.38$\% \\
...
\paragraph{One result we do not claim.} At $x=250$ the release arm beats \emph{never releasing} on 4 of 5 seeds with paired-$t$ $p=0.041$, which would clear the bar we set for that comparison in \S\ref{sec:removal}. We reached it, however, by testing seven release points and selecting the best; with seven comparisons the appropriate threshold is nearer $p<0.007$. We therefore continue to report that releasing does not beat retaining, on multiple-comparisons grounds rather than on the number itself.
...
\begin{table}[H]
\centering
\caption{The floor sweep (20k steps, 3.4M, five matched seeds, ramp $=x$). $p_{\text{perm}}$ is the exact one-sided sign-flip permutation $p$ (floor $1/32$). No run diverged in 15.}
\label{tab:floor}
\begin{tabular}{lccc}
\toprule
...
\midrule
Mean bpb & 0.9017 & 0.8905 & \textbf{0.8836} \\
vs baseline & $-0.17$\% & $+1.07$\% & $\mathbf{+1.84}$\% \\
Seeds positive & 3/5 & 5/5 & 5/5 \\
$p_{\text{perm}}$ & 0.656 & \textbf{0.031} & \textbf{0.031} \\
...
\begin{itemize}
    \item \textbf{Releasing does not beat retaining, and this is now a measurement rather than a gap.} Experiment~(4) built the fixed evaluation harness precisely to settle this, and it did: with the paired residual at $0.0011$ bpb---below the $0.0012$ we pre-registered as the threshold for adequate power at $n=5$---the minimum detectable effect is $0.00139$ bpb against a hypothesised ${\approx}0.0015$. An effect of that size would have been detected; the observed difference is $+0.00028$ ($3/5$, paired-$t$ $0.6096$). ``Removal costs nothing'' stands; ``removal helps'' is excluded. \emph{What experiment~(4) did not fix:} arm~F has no seed-42 checkpoint, so under the fixed harness it is $n=4$, where the exact permutation test floors at $0.062$ and $p<0.05$ is unreachable by construction. Experiment~(1)'s criterion as literally written---on $B-F$---therefore still cannot be met, and the $n=5$ ramp arm carries the conclusion.
...
\emph{Outcome of the comparison this experiment existed to enable.} Both primary claims survive the harness change at full strength: $A-B$ is $5/5$ positive ($p_{\text{perm}}=0.031$, paired-$t$ $0.0028$, $d_z=2.92$, $+1.36$\%) and $A-R$ is $5/5$ ($p_{\text{perm}}=0.031$, paired-$t$ $0.0037$, $d_z=2.72$, $+1.40$\%). The open question is now \emph{answered} rather than unresolved. We pre-registered that reaching $p<0.05$ at $n=5$ required the paired residual below $0.0012$; it is $0.001118$, so the comparison is adequately powered for the first time. At that precision the minimum detectable effect is $0.00139$ bpb, and we hypothesised the true effect at ${\approx}0.0015$---an effect that size would have been detected. None was: $B-R$ is $3/5$ positive with a mean of $+0.00028$ bpb ($p_{\text{perm}}=0.344$, paired-$t$ $0.6096$, $d_z=0.25$), five times smaller than hypothesised. ``Removal costs nothing'' is confirmed; ``removal helps'' is \emph{excluded}, not merely unmeasured.
...
The two results we would most want a reader to take away are the ones that bound the claim rather than extend it. First, the constraint has a \emph{minimum} duration as well as a maximum useful one: released at $0.25$\% of training it stops working, and on two of five seeds leaves the model worse than never applying it at all. The recipe is therefore bounded on both sides, and undershooting is far more costly than overshooting. Second, releasing the constraint does not \emph{beat} retaining it. That comparison was unresolved for most of this project's life, and it is now excluded rather than merely unmeasured: a fixed evaluation set cut the paired residual to $0.0011$ bpb, at which an effect of the size we hypothesised would have been detected, and none was. The case for releasing is that it costs nothing and lets one ship a standard architecture---not that it makes a better model.
