...
\subsection{When to release: the curriculum is short}
...
\paragraph{Interpretation.} That a tenth of training suffices, and that longer application is mildly worse, fits the mechanism in \S\ref{sec:mechanism}: the constraint does its work while the hierarchy is being laid down, and after that it is a mild restriction on the upper layers. It is also consistent with the 100k trace in Appendix~\ref{app:100k}, where the baseline-to-quartic gap peaks at 10k steps and narrows thereafter. What we have not tested is the other side of the minimum: whether releasing earlier than 2k still works, or whether the benefit collapses below some duration.
