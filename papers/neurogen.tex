...
\begin{table}[H]
\centering
\caption{Removing windows at the halfway point (20k steps, 3.4M, five matched seeds; init and data order shared within each seed). Arms A and B are the validated runs of Table~\ref{tab:3_4m}; only arm~F is trained here. The $\Delta$ columns are relative val\_bpb improvements over that seed's own arm.}
...
The two absorbing interventions trade update magnitude for time: both cut the peak update roughly fourfold but leave the model in the shocked state ${\sim}2.5\times$ longer (28 steps to ${\sim}70$). The ramp pays neither cost and arrives at the same loss. We report this as a mechanism result, not a validated recipe: these are single-seed runs stopped 1000 steps after the switch, so they establish what causes the shock, not that any treatment improves converged val\_bpb or prevents the one-in-eight divergence---detecting the latter would require many runs.
