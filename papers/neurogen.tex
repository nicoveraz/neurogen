...
\begin{itemize}
    ...
    \item \textbf{An honest scale probe and a pre-registration.} $+2.6$\% at 125M across 5 paired seeds with one negative, and a stated protocol for the three experiments that would make the curriculum claim publishable (\S\ref{sec:scale}, \S\ref{sec:limitations}).
\end{itemize}
...
\paragraph{The divergence.} Arm~F at seed 256 produced NaN after the switch and is reported here rather than excluded. The learning-rate schedule is ruled out by inspection: it is a single cosine over the full horizon with no discontinuity at the switch point. Three candidate causes remain---(i) the attention implementation changes at the switch, from an explicit masked softmax to a fused causal kernel; (ii) the Adam second-moment estimate is stale, having been accumulated under window-8 gradients, so the first post-switch updates can be large for parameters with small historical second moment, which gradient-norm clipping does not prevent; (iii) the layer-0 softmax denominator jumps from 8 terms to up to 256 in a single step. These are distinguishable: replacing the switch target with an explicit all-full window list keeps the masked-softmax implementation and changes only the mask, isolating (i) from (ii) and (iii). The intermediate training curve for the diverged run was not retained, so this requires re-running with per-step instrumentation; the protocol is specified in \S\ref{sec:limitations}. A divergence rate of 1 in 3 at the switch point is a real property of the naive recipe and we do not present the curriculum as free of it.
...
\begin{itemize}
    \item \textbf{The central result rests on two seeds.} Arm~F completed on seeds 42 and 137 only; seed 256 diverged. The F-vs-A direction is consistent ($2/2$) but cannot be significant at $n=2$, where the exact paired permutation test floors at 0.25.
    \item \textbf{F vs B is sign-split} and is not claimed (\S\ref{sec:removal}).
    \item \textbf{The removal recipe diverged on 1 of 3 seeds} and the cause is not yet established.
...
\paragraph{(2) Divergence diagnosis.} Re-run seed 256 with per-step loss, pre-clip gradient norm, and maximum parameter update logged for steps $[\text{switch}-100, \text{switch}+500]$. Discriminate cause (i) from (ii)/(iii) by switching to an explicit all-full window list, which preserves the masked-softmax implementation and changes only the mask. Then test three interventions: a 200-step LR re-warmup after the switch, resetting the optimizer state at the switch, and ramping the window linearly over 500 steps instead of switching discontinuously. \emph{Criterion:} name which candidate reproduces the NaN and which intervention prevents it.
