...
\paragraph{How short can it be?} Since 2k steps sufficed and shorter was better, we swept further down---1000, 500 and 250 steps, or 5\%, 2.5\% and 1.25\% of training---at the same five seeds. The learning-rate cosine is at its peak at all of these points as well as at 2k, so unlike the 10k-versus-15k comparison these are compared at identical learning rate. The result is that \emph{the benefit never collapses}. Every one of the seven release points beats its paired baseline on all five seeds ($p=0.031$ throughout, $d_z$ between 2.12 and 2.92), and the shortest is the best: releasing after 250 steps gives $+1.91$\%, the largest effect we measure anywhere in this work. With the 500-step ramp, that model is at full attention from step 750 of 20{,}000 and trains the remaining 19{,}250 steps unconstrained.

\begin{table}[H]
\centering
\caption{The full release curve (20k steps, 3.4M, five matched seeds). $R@x$ releases the windows at step $x$; every point beats its paired baseline on 5/5 seeds. No lower bound was found.}
