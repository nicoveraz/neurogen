...
A 2D topographic regularizer that forces digits to be grid-adjacent may **help Phase 1** (category formation — the grid encodes "digits belong together") but **hurt Phase 2** (ordinal refinement — the grid constrains the axis along which digits can spread). A testable consequence: in the topographic condition, the ordinal correlation at 100K should be *weaker* than in the baseline (less negative r). If observed, it's direct evidence that the topographic constraint is trading category formation against within-category refinement.
