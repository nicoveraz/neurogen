...
| 2 | ❌ abandoned (Option C) | `reports/exp2/gradient_vanishing_analysis.md` | Four pilots, three pathologies, unified gradient-vanishing diagnosis. Not pursuing further loss formulations. Finding folded into the writeup as §4. |
| writeup | 📝 draft complete | `reports/writeup/draft.md` | Option C consolidation. ~8500 words, 6 sections + 5 appendices. Ready for review. |
| quartic-emergence | 🚧 training overnight | pending `runs/exp_quartic/analysis.out` | Test whether quartic-window architecture produces emergent topographic-like embedding organization without explicit loss. 100K steps, same cadence as Exp 0. Watcher auto-runs comparison when training exits. |
...
| **Gaussian-kernel topographic losses have no stable attractor at non-zero distance.** Gradient vanishes at both d→0 and d→∞; equilibria are stationary points but not attractors. For gradient-based learning of positions, use distance-based losses (MSE on `d_ij`), not kernel-based. | Exp 2 pilots 1/2A/2B/2C/2D | `feedback_topo_loss_equilibrium.md`, full analysis in `reports/exp2/gradient_vanishing_analysis.md` |
| **Pilot-loop momentum.** After three distinct pilot failure modes on the same experiment, step back and analyze — each new pilot fixing the previous pilot's pathology is a signal about the design, not calibration progress. | Exp 2 pilot sequence | `feedback_pilot_loop_momentum.md` |
