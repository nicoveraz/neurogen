...
| 2 | ⏸ paused pending design review | `reports/exp2/gradient_vanishing_analysis.md` | Four pilots across three loss formulations each revealed distinct pathologies; unified analysis shows Gaussian-kernel-based topographic losses cannot produce stable attractors regardless of target matrix design. Decision pending between (A) distance-based formulation, (B) initialization-only topography, (C) skip Exp 2, write up Exp 0 + Exp 1 + the gradient-vanishing finding as a first paper. Sitting with the call rather than iterating further. |
